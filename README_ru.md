...
- **Вложения файлов** — текстовые файлы передаются в opencode через `-f` (лимиты размера/числа)
- **Безопасные скрипты** — ответ с shebang сохраняется как `.sh`; запуск только после подтверждения (или `ASK_AI_AUTO_EXEC`)
...
| **Пакетная обработка фото** | `Напиши скрипт для обрезки фото до 1:1` | AI пишет bash-скрипт → сохраняется `.sh` → подтверждение запуска |
| **Коллаж / слайд-шоу** | `Напиши скрипт для создания коллажа из этих фото` | AI генерирует скрипт ImageMagick/ffmpeg; запуск по подтверждению |
| **Обрезка по пропорции** | `Напиши скрипт для обрезки фото до соотношения 16:9` | Скрипт рядом с файлами; автозапуск опционален |

> **Обработка скриптов:** если ответ начинается с shebang (`#!/bin/bash`, …) или code fence с shebang, runner сохраняет `.sh` и спрашивает **Запустить этот скрипт? [y/N]**. `ASK_AI_AUTO_EXEC=1` — всегда запускать, `ASK_AI_AUTO_EXEC=0` — никогда. Информационные ответы только показываются.
...
| `ASK_AI_CLIPBOARD` | пусто | `1` — копировать ответ в буфер обмена |
| `ASK_AI_MAX_ATTACH_BYTES` | `204800` | Макс. размер файла для `-f` |
| `ASK_AI_MAX_ATTACH_FILES` | `20` | Макс. число прикрепляемых текстовых файлов |
| `GLOW_DISABLED` | пусто | `1` — вывод без glow (`askr`) |
| `ASK_AI_LOCALE` | авто (`$LANG`) | Язык: `ru_RU` / `en_EN` |
| `ASK_AI_THEME` | авто (палитра) | Тема: `dark` / `light` |
| `TERMINAL` | пусто | Предпочитаемый терминал (приоритет над konsole) |

**Примеры:**

```bash
export ASK_AI_MODEL="opencode/deepseek-v4-flash"
export ASK_AI_EFFORT="max"
export ASK_AI_MODE="plan"
export ASK_AI_AUTO_EXEC="0"    # никогда не запускать сгенерированные скрипты
export ASK_AI_CLIPBOARD=1
export GLOW_DISABLED=1
export ASK_AI_LOCALE="ru_RU"
export ASK_AI_THEME="dark"
```

Подробнее о языке в разделе [Локализация](#локализация). Тема определяется из схемы KDE и работает для диалога и заголовка раннера.
